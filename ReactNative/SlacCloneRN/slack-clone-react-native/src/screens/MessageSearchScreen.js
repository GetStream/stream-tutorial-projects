...
export const MessageSearchScreen = () => {
  ...
  const addToRecentSearches = async q => {
    ...
    const sliced = _recentSearches.slice(0, 7);
    setRecentSearches(sliced);
    await AsyncStore.setItem('@slack-clone-recent-searches', sliced);
  };
  ...
  useEffect(() => {
    const loadRecentSearches = async () => {
      const recent = await AsyncStore.getItem(
              ...
              <FlatList
                ...
                renderItem={({item, index}) => (
                  <TouchableOpacity
                    ...
                    <SCText onPress={() => removeFromRecentSearches(index)}>
                      X
                    </SCText>
                  </TouchableOpacity>
                )}
                ...
                renderItem={({item}) => (
                  <TouchableOpacity
                    ...
                    <View style={styles.resultMessageContainer}>
                      <SCText style={{fontWeight: '700', fontSize: 14}}>
                        {item.user?.name}
                      </SCText>
                      <SCText style={{fontSize: 14, marginTop: 2}}>
                        {item.text}
                      </SCText>
                    </View>
                  </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  headerContainer: {flexDirection: 'row', width: '100%', padding: 10},
  inputBox: {
    flex: 1,
    margin: 3,
    padding: 10,
  ...
  resultCountContainer: {padding: 15, borderBottomWidth: 0.5},
  ...
  recentSearchesTitle: {padding: 5, fontSize: 13},
  ...
  listEmptyContainer: {flex: 1, alignItems: 'center', justifyContent: 'center'},
  resetButton: {
    padding: 15,
    paddingTop: 10,
    paddingBottom: 10,
  ...
  resultItemContainer: {padding: 10},
  resultChannelTitle: {
    paddingTop: 10,
    paddingBottom: 10,
    fontWeight: '700',
  ...
  resultMessageContainer: {paddingLeft: 10},
});
