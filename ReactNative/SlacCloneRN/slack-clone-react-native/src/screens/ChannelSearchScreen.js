...
export const ChannelSearchScreen = () => {
  ...
  const channelsOnly = params?.channelsOnly ?? false;
  ...
  const onChangeText = async newText => {
    ...
    const result = await chatClient.queryChannels({
      type: 'messaging',
      $or: [
        {'member.user.name': {$autocomplete: newText}},
        {name: {$autocomplete: newText}},
      ],
    });
    setResults(result);
  };
      ...
      <View>
            ...
            <FlatList
              ...
              renderItem={({item}) => (
                <TouchableOpacity
                  ...
                  <SCText style={styles.memberName}>
                    {item.data?.name || 'DM'}
                  </SCText>
                </TouchableOpacity>
              )}
        ...
        <View style={styles.searchResultsContainer}>
          ...
          <FlatList
            ...
            keyboardShouldPersistTaps="always"
            data={results}
            renderItem={({item}) => renderChannelRow(item)}
          />
        </View>
      </View>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  cancelButton: {alignSelf: 'center', padding: 5},
  ...
  memberName: {marginTop: 5, fontSize: 10, textAlign: 'center'},
  searchResultsContainer: {paddingTop: 10},
