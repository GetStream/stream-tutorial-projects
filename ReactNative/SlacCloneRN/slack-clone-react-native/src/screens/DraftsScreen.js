...
export const DraftsScreen = () => {
  ...
  useEffect(() => {
    const getDraftMessages = async () => {
      ...
      setResults(drafts.filter(r => r && r.text));
    };
    getDraftMessages();
  }, []);
        ...
        <FlatList
          ...
          renderItem={({item}) => (
            <TouchableOpacity
              ...
              <SCText
                style={[styles.draftChannelTitle, {color: colors.boldText}]}>
                {item.title}
              </SCText>
...
const styles = StyleSheet.create({
  ...
  draftChannelTitle: {fontWeight: 'bold'},
  draftMessageText: {marginTop: 10, fontWeight: '400'},
  backIcon: {fontSize: 35, textAlign: 'left'},
